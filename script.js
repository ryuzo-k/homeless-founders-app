...
async function updateHomeStats() {
    try {
        log('🔄 updateHomeStats called');
        log('🔧 SupabaseDB available:', typeof SupabaseDB !== 'undefined');
        
        if (typeof SupabaseDB !== 'undefined') {
            log('📡 Fetching houses from Supabase...');
            // Supabaseからデータを取得
            const houses = await SupabaseDB.getHackerHouses();
            log('🏠 Houses fetched:', houses);
            ...
            log('📊 Calculated stats - Houses:', totalHouses, 'Regions:', activeRegions);
            ...
            log('🎯 DOM elements found - Houses:', !!homeHousesEl, 'Regions:', !!homeRegionsEl);
            
            if (homeHousesEl) {
                homeHousesEl.textContent = totalHouses;
                log('✅ Updated houses display to:', totalHouses);
            }
            if (homeRegionsEl) {
                homeRegionsEl.textContent = activeRegions;
                log('✅ Updated regions display to:', activeRegions);
            }
            
            log('📊 Stats updated - Houses:', totalHouses, 'Regions:', activeRegions);
        } else {
            log('⚠️ SupabaseDB not available, using fallback');
            ...
            log('📊 Fallback stats - Houses:', totalHouses, 'Regions:', activeRegions);
        }
    } catch (error) {
